...
parser.add_option("--training", dest="training", help="training set or validation set", type="string")
...
def infinite_generator_from_one_volume(config, img_array):
    ...
    while True:
        ...
        #print(f"shape : {slice_set[num_pair].shape}")
        num_pair += 1
...
def get_self_learning_data(fold, config):
    ...
    for index_subset in fold:
        ...
        #print(f'voilà le problème : {os.path.join(luna_subset_path, "*.mhd")}')
        file_list = os.listdir(config.DATA_DIR)
        for img_file in tqdm(file_list):
            itk_img = sitk.ReadImage(f'{config.DATA_DIR}/{img_file}') 
            ...
            if x is not None:
                slice_set.extend(x)
    print(f'len : {len(slice_set)}')
    print(f'return shape : {np.array(slice_set).shape}')
