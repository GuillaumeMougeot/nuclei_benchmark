...
for i,fold in enumerate(tqdm(conf.train_fold)):
	...
	print(file_name)
	s = np.load('generated_cubes/bat_32_s_64x64x32_6-001.npy')
	x_train.extend(s)
"""

s = np.load('generated_cubes/trainset.npy')
print(f's load : {s.shape}')
x_train.extend(s)
...
num = 0
for i in range(10) :
	tifffile.imwrite(f'toto{num}.tif', np.squeeze(x_train[i]))
	num = num + 1
...
for i,fold in enumerate(tqdm(conf.valid_fold)):
    ...
    x_valid.extend(s)
"""
s = np.load('generated_cubes/validset.npy')
x_valid.extend(s)
...
for epoch in range(intial_epoch,nb_epoch):
	...
	for iteration in range(int(x_train.shape[0]//conf.batch_size)):
		...
		if (iteration + 1) % 5 ==0:
			print('Epoch [{}/{}], iteration {}, Loss: {:.6f}'
				.format(epoch + 1, nb_epoch, iteration + 1, np.average(train_losses)))
			sys.stdout.flush()
	...
	train_losses=[]
	valid_losses=[]
	print(f"Temps d'exécution actuel : {time.time() - start}")
...
print(f"Temps d'exécution final : {time.time() - start}")